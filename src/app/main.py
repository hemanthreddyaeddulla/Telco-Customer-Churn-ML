...
from src.serving.inference import predict_full  # Core ML inference logic
...
class CustomerData(BaseModel):
    ...
    SeniorCitizen: int = 0  # 0 or 1 - is the customer a senior citizen
...
def get_prediction(data: CustomerData):
    ...
    try:
        ...
        # Returns label, churn_probability, threshold, top SHAP drivers, action.
        return predict_full(data.model_dump())
...
def gradio_interface(
    ...
    # Call same inference pipeline as the API endpoint.
    res = predict_full(data)

    # Echo the submitted selections so it's clear the prediction matches the inputs.
    lines = ["YOUR SELECTIONS"]
    lines += [f"   - {key}: {value}" for key, value in data.items()]
    lines += [
        "",
        "PREDICTION (based on the selections above)",
        f"   Prediction: {res['prediction']}",
        f"   Churn probability: {res['churn_probability']:.1%} "
        f"(decision threshold {res['threshold']:.0%})",
        "",
        "   Top drivers (SHAP):",
    ]
    for d in res["top_drivers"]:
        lines.append(f"   - {d['feature']} ({d['direction']}, {d['impact']:+.3f})")
    lines += ["", f"   Recommended action: {res['recommended_action']}"]
    return "\n".join(lines)
...
demo = gr.Interface(
    ...
    inputs=[
        ...
        # Demographic flag (0 = not senior, 1 = senior)
        gr.Dropdown([0, 1], label="Senior Citizen", value=0),
    ],
    outputs=gr.Textbox(label="Churn Prediction", lines=34),
    title="Telco Customer Churn Predictor",
    description="""
    **Predict customer churn probability using machine learning.**

    Fill in the customer details below to get a churn prediction. The model is an XGBoost
    pipeline trained on historical telecom customer data to identify customers at risk of churning.
...
app = gr.mount_gradio_app(
    ...
    path="/",  # serve the Gradio app at the root so the Hugging Face Space shows it
)

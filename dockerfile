...
# at /app/model (+ the threshold contract at /app/). --tune runs Optuna so the deployed
# model is the same tuned model as the notebook (costs a few extra build minutes).
RUN python scripts/fetch_data.py \
    && python scripts/run_pipeline.py --input data/raw/WA_Fn-UseC_-Telco-Customer-Churn.csv --tune \
